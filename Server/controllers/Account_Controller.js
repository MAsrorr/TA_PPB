...
async function login(req, res) {
    try {
        ...
        if (role === 'peserta_magang' || role === 'admin') {
            const userModel = role === 'peserta_magang' ? models.peserta_magangss : models.admins;
...
const refreshToken = async (req, res) => {
    ...
    try {
        ...
        jwt.verify(refreshTokenCookie, process.env.JWT_KEY, async (err, decoded) => {
            ...
            const user = await models.peserta_magangss.findOne({ where: { username: decoded.username } }) || await models.admins.findOne({ where: { username: decoded.username } });
            if (!user) {
                return res.status(403).json({
