...
function addAdmin(req, res,){
    models.admins.findOne({where:{username: req.body.username}}).then(result =>{
        if (result){
            ...
            models.peserta_magangss.findOne({where:{username: req.body.username}}).then(result =>{
                if (result){
                    res.status(409).json({
                        message: 'dah ada email bang'
                    })
...
function showPeserta(req, res){
    ...
    models.peserta_magangss.findByPk(id).then(result =>{
        res.status(200).json({
...
async function showPesertaAll(req, res){
    statusCheck(req, res);
    await models.peserta_magangss.findAll().then(result =>{
        res.status(200).json({
            peserta_magang:result
        });
    }).catch(error =>{
...
async function showPesertaAktifAll(req, res){
    ...
    await models.peserta_magangss.findAll({where:{
      status_aktif:true, 
      tanggal_mulai: {
        [Op.lte]: currentDate
      }
...
async function showPesertaAlumniAll(req, res){
    statusCheck(req, res);
    await models.peserta_magangss.findAll({where:{status_aktif:false}}).then(result =>{
        res.status(200).json({
            peserta_magang:result
        });
    }).catch(error =>{
...
async function showCalonPesertaAll(req, res) {
    ...
    await models.peserta_magangss.findAll({
        where: {
            tanggal_mulai: {
    ...
    bcryptjs.genSalt(10,async function(err,salt){
        bcryptjs.hash(req.body.password,salt,async function(err,hash){
            try {
                ...
                console.log("cek2");
                await models.peserta_magangs.update(updatedPeserta, {where:{id:id}});
                console.log("cek");
...
function deletePeserta(req, res){
    ...
    models.peserta_magangs.destroy({where:{id:id}}).then(result =>{
        res.status(200).json({
...
async function showPresensiPerDay(req, res) {
    ...
    try {
        const presensi = await models.peserta_magangs.findAll({
            include: [{
...
function showTugasStatusByTugas(req, res){
    ...
    models.peserta_magangs.findAll({
        where: {
            status_aktif:true
        },
        include:[{
...
async function addStatusToAll(result_tugas, req, res) {
    try {
      const peserta = await models.peserta_magangs.findAll({ where: { status_aktif: true } });
...
async function statusCheck(req, res){
    try {
        ...
        const outdatedPeserta = await models.peserta_magangs.findAll({
          where: {
            tanggal_selesai: {
              [Op.lt]: currentDate,
            },
...
async function exportPeserta(req, res) {
    try {
      statusCheck(req,res);
      const results = await models.peserta_magangs.findAll();
  ...
  async function exportPesertaAktif(req, res) {
    try {
      ...
      const response = await axios.get('https://worldtimeapi.org/api/timezone/Asia/Jakarta');
      const tanggal = moment.tz(response.data.datetime, 'Asia/Jakarta');
      const results = await models.peserta_magangs.findAll({where:{
        status_aktif:true,
        tanggal_mulai: {
          [Op.lte]: tanggal
        }
  ...
  async function exportPesertaAlumni(req, res) {
    try {
      statusCheck(req,res);
      const results = await models.peserta_magangs.findAll({where:{status_aktif:false}});
  ...
  try {
    ...
    const ambilNama = await models.peserta_magangs.findByPk(id);
    const response = await axios.get('https://worldtimeapi.org/api/timezone/Asia/Jakarta');
    const tanggal = moment.tz(response.data.datetime, 'Asia/Jakarta');
